...
@interface MWPhotoBrowser ()<CLImageEditorDelegate,UIPrintInteractionControllerDelegate> {
    int checkViewAppear;
    id <MWPhoto> selectedPhoto;
    id <MWPhoto> everNotePhoto;
    
    MWPhoto *photoChanged;
...
- (void)performLayout {
    ...
    self.navigationItem.rightBarButtonItems=[NSArray arrayWithObjects:_nextButton,_previousButton, nil];
    // Update nav
...
- (void)viewDidAppear:(BOOL)animated {
    NSLog(@"viewDidAppear3,,%d",_currentPageIndex);
    ...
    }else
    {
        ...
        NSLog(@"viewDidAppear1,,%d",_currentPageIndex);
        [self jumpToPageAtIndex:[_photos indexOfObject:photoChanged]animated:NO];
        //[self loadAdjacentPhotosIfNecessary:photoChanged];
    ...
    if (actionSheet.tag == ACTION_SHEET_OLD_ACTIONS) {
        ...
        switch (buttonIndex) {
            case 0:
            {
                ...
                session.isAuthenticated?[[EvernoteSession sharedSession] logout]:[self authenticateEverNote];
                
            }
                break;
            case 1:
            {
                [self evernoteViewCreation:everNotePhoto];
            }
...
#pragma mark -Addition To View
- (void)bottomToolBarCreation {
    UIView *toolView=[[UIView alloc]initWithFrame:CGRectMake(0,self.view.bounds.size.height-60, self.view.bounds.size.width, 60)];
    toolView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight |UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleLeftMargin;
    
    [self.view addSubview:toolView];

    //UIImage *image = [UIImage imageNamed: @"Safari-Small.png"];
    NSMutableArray *array = [NSMutableArray array];
    //initWithTitle:i==0?@"EverNote":i==1?@"twitter":i==2?@"AirPrint":i==3?@"Email":i==4?@"Save":i==5?@"Copy":@"Edit"
    for (int i=0; i<7; i++) {
        AAActivity *activity = [[AAActivity alloc] initWithTitle:nil image:[UIImage imageNamed:i==0?@"ImgeEdit.png":i==1?@"saveImage":i==2?@"CopyImage.png":i==3?@"printImage.png":i==4?@"mailImage.png":i==5?@"twitterImage.png":@"EvernoteImage.png"]
                                                     actionBlock:^(AAActivity *activity, NSArray *activityItems) {
                                                         ...
                                                         [self setActionOnControllSheet:i];
                                                     }];
        [array addObject:activity];
    }
    
    AAActivityAction *activityActionView = [[AAActivityAction alloc] initWithActivityItems:@[@"http://www.apple.com/"]
                                                   applicationActivities:array
                                                               imageSize:AAImageSizeSmall];
    [activityActionView showInView:toolView];
    // [_toolbar addSubview:activityActionView];
    
}
-(void)setActionOnControllSheet:(int)tageNo
{
    if (tageNo == 0) {
        [self extractPhoto:8];
    ...
    else if(tageNo == 3||tageNo == 5||tageNo == 6){
        [self extractPhoto:tageNo];
    }
    
}
#pragma mark -ExtractActualImage

- (void)extractPhoto:(int)tage {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    selectedPhoto =(MWPhoto *)photo;
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Preparing", @"Displayed with ellipsis as 'Preparing...' when an item is in the process of being prepared")]];
        tage==6? [self performSelector:@selector(createActionSheet:) withObject:photo afterDelay:0]:tage==8?[self performSelector:@selector(actualEditPhoto:) withObject:photo afterDelay:0]:tage==5?[self performSelector:@selector(actuallyTwitterPhoto:) withObject:photo afterDelay:0]:[self performSelector:@selector(actuallyPrintPhoto:) withObject:photo afterDelay:0];
    }
}


-(void)actualEditPhoto:(id<MWPhoto>)photo
{
    [self hideProgressHUD:NO];
    checkViewAppear=8;
    CLImageEditor *editor = [[CLImageEditor alloc] initWithImage:[photo underlyingImage]];
...
{
    //id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    photoChanged=[MWPhoto photoWithImage:image];
    [_photos replaceObjectAtIndex:_currentPageIndex withObject:photoChanged];
    [self jumpToPageAtIndex:_currentPageIndex==0?_currentPageIndex+1:_currentPageIndex-1 animated:NO];
    
}
#pragma mark -AirPrint
-(void)actuallyPrintPhoto:(id<MWPhoto>)photo
{
    
    [self hideProgressHUD:NO];
    NSData *myData = [self compressImageInData:[photo underlyingImage]];
...
-(void)actuallyTwitterPhoto:(id<MWPhoto>)photo
{
    SLComposeViewController *composeController = [SLComposeViewController composeViewControllerForServiceType:SLServiceTypeTwitter];
    
    [composeController setInitialText:@""];
    [composeController addImage:[self compressImage:[photo underlyingImage]]];
...
-(void)evernoteViewCreation:(id<MWPhoto>)photo
{
    everNotePhoto=photo;
    EvernoteSession *session = [EvernoteSession sharedSession];
    checkViewAppear=!session.isAuthenticated?8:2;
    [session authenticateWithViewController:self completionHandler:^(NSError *error) {
        if (error || !session.isAuthenticated) {
            ...
            [self createPhotoNote:photo];
        }
    }];
    
}
- (IBAction)createPhotoNote :(id<MWPhoto>)photo{
    
    
    NSData *myFileData = [self compressImageInData:[photo underlyingImage]];//UIImageJPEGRepresentation([photo underlyingImage],0.1);
    ...
    [[EvernoteNoteStore noteStore] createNote:newNote success:^(EDAMNote *note) {
        ...
        [self getUserShardIdWithNote:note];
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Message"
                                                        message:[NSString stringWithFormat:@"Error creating note : %@",error]
                                                       delegate:nil
                                              cancelButtonTitle:@"OK"
                                              otherButtonTitles:nil];
        [alert show];
...
-(void)createActionSheet:(id<MWPhoto>)photo
{
    [self hideProgressHUD:YES];
    everNotePhoto=photo;
    EvernoteSession *session = [EvernoteSession sharedSession];
    NSString *logString =session.isAuthenticated?@"SignOut":@"SignIn";
    UIActionSheet *evernoteAction= [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                      cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
                                                      otherButtonTitles:NSLocalizedString(logString, nil), NSLocalizedString(@"Add Note", nil), nil];
    [evernoteAction showInView:self.view];
...
-(NSData *)compressImageInData:(UIImage *)image{
    float actualHeight = image.size.height;
    float actualWidth = image.size.width;
    float maxHeight = 600.0;
    float maxWidth = 800.0;
...
- (void)getUserShardIdWithNote:(EDAMNote *)note {
    ...
    [[EvernoteUserStore userStore] getUserWithSuccess:^(EDAMUser *user) {
        ...
        [self shareSingleNoteWithNoteGUID:note.guid withShardId:shardId];
