...
@property (nonatomic, strong) NSMutableArray *notThisMonthButtons;
...
@property (nonatomic, strong) NSDateFormatter *dayFormatterCreated;
...
{
    for (id dayButton in self.contentView.subviews) {
        if ([dayButton isKindOfClass:[UIButton class]]) {
    ...
    [self.dayButtons removeAllObjects
...
- (void)setBeginningDate:(NSDate *)date;
{
    ...
    for (NSUInteger index = 0; index < self.daysInWeek; index++) {
        ...
        [(UIButton *)self.dayButtons[index] setHidden:YES];
        [(UIButton *)self.notThisMonthButtons[index] setHidden:YES];
        
        NSInteger thisDayMonth = thisDateComponents.month;
        if (self.monthOfBeginningDate != thisDayMonth) {
            [(UIButton *)self.notThisMonthButtons[index] setHidden:NO];
        } else {
...
//            } else {
                ...
                [self setDateCellImageWithButton:self.dayButtons[index] dateForButton:date];
...
-(void)setImageForDateWithArray:(NSMutableArray *)array beginDate:(NSDate *)dateValue{
    NSDateComponents *offset = [NSDateComponents new];
    offset.day = 1;
    for (NSUInteger index = 0; index < self.daysInWeek; index++) {
    [self.dayButtons[index] setBackgroundImage:nil forState:UIControlStateNormal];
    dateValue = [self.calendar dateByAddingComponents:offset toDate:dateValue options:0];

    }

}
-(void)clearButtonBackGround{
    [self.dayButtons enumerateObjectsUsingBlock:^(UIButton *obj, NSUInteger idx, BOOL *stop) {
...
- (void)setBeginningDate:(NSDate *)date;
{
 ...
 [(UIButton *)self.dayButtons[index] setBackgroundImage:nil forState:UIControlStateNormal];
 [self.arrayImages enumerateObjectsUsingBlock:^(GTLDriveFile *obj, NSUInteger idx, BOOL *stop) {
 ...
 {
 [(UIButton *)self.dayButtons[index] setBackgroundImage:[UIImage imageNamed:@"imgIcon.png"] forState:UIControlStateNormal];
 [GoogleDriveManager getIconWithFile:obj completionBlock:^(NSData *data, NSError *error) {
 if (!error) {
 [(UIButton *)self.dayButtons[index] setBackgroundImage:[UIImage imageWithData:data] forState:UIControlStateNormal];
 }
...
//-(NSDateFormatter *)dayFormatterCreated{
//    if (!_dayFormatterCreated) {
//        _dayFormatterCreated = [NSDateFormatter new];
//        _dayFormatterCreated.calendar = self.calendar;
//        _dayFormatterCreated.dateFormat = @"yyyy-MM-dd";
//    }
//    return _dayFormatterCreated;
//    
//}
...
-(NSString *)createdDateForImageInCalandarCell:(GTLDriveFile *)driveFileImage{
    NSString *resultDate=driveFileImage.imageMediaMetadata.date.length>0?driveFileImage.imageMediaMetadata.date: driveFileImage.createdDate.stringValue;
    
   // NSLog(@"My date is = %@",resultDate);
    return resultDate;
}
-(void)setDateCellImageWithButton:(UIButton *)button dateForButton:(NSDate *)dateButton{

    
    [self.arrayImages enumerateObjectsUsingBlock:^(GTLDriveFile *obj, NSUInteger idx, BOOL *stop) {
        if ([[self.dayFormatterImage stringFromDate:obj.modifiedDate.date] isEqualToString:[self.dayFormatterImage stringFromDate:dateButton]])
        {
            [button setBackgroundImage:nil forState:UIControlStateNormal];
            NSLog(@"Date,,%@..%@",[self.dayFormatterImage stringFromDate:obj.modifiedDate.date],[self.dayFormatterImage stringFromDate:dateButton]);
//            [button setTitle:@"jj" forState:UIControlStateNormal];
//            [button setBackgroundColor:[UIColor redColor]];
//            [button setBackgroundImage:[UIImage imageNamed:@"imgIcon.png"] forState:UIControlStateNormal];
            [GoogleDriveManager getIconWithFile:obj completionBlock:^(NSData *data, NSError *error) {
                [button setBackgroundImage:nil forState:UIControlStateNormal];
