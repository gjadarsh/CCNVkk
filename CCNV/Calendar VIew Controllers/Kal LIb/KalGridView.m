...
CGSize kTileSize;
CGSize kTileSize_ipad;

//CGSize kTileSize = { 109.71f, 75.f };
//CGSize kTileSize_ipad = { 109.71f, 75.f };

//const CGSize kTileSize = { 46.f, 44.f };
//const CGSize kTileSize_ipad = {110.f, 84.f};
...
- (id)initWithFrame:(CGRect)frame logic:(KalLogic *)theLogic delegate:(id<KalViewDelegate>)theDelegate
{
  ...
  // MobileCal does the same thing.
    ...
    if(isIpad){
        ...
        if ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft || [UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeRight) {
            ...
            kTileSize.width = 146.50f;
            kTileSize.height = 105.f;
            kTileSize_ipad.width = 146.50f;
            kTileSize_ipad.height = 105.f;
        }
        else{
            kTileSize.width = 109.71f;
            kTileSize.height = 148.f;
            kTileSize_ipad.width = 109.71f;
            kTileSize_ipad.height = 148.f;
        }
        
        frame.size.width = 7 * kTileSize_ipad.width;
    }
    else{
        CGSize result = [[UIScreen mainScreen] bounds].size;
        if ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft || [UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeRight)
        {          
            kTileSize.width = result.height/7;
            kTileSize.height = 34.3f;
            kTileSize_ipad.width = result.height/7;
            kTileSize_ipad.height = 34.3f;
        }
        else
        {
            kTileSize.width = 46.0f;
            kTileSize.height = (result.height - 136)/6;
            kTileSize_ipad.width = 46.0f;
            kTileSize_ipad.height = (result.height - 136)/6;
        }
  ...
  if (self = [super initWithFrame:frame]) {
    ...
    delegate = theDelegate;
      ...
      NSLog(@"kal view frame %@",NSStringFromCGRect(self.frame));
  ...
  if ([hitView isKindOfClass:[KalTileView class]]) {
    ...
    if (tile.belongsToAdjacentMonth) {
      ...
      self.selectedTile = [frontMonthView tileForDate:tile.date];
        ...
        if(tile.marked){
            isLoadImgForSelectedDate=TRUE;
            Caldate=[NSString stringWithFormat:@"%04i-%02i-%02i",[tile.date year],[tile.date month],[tile.date day]];
...
- (void)slide:(int)direction
{
  ...
  [self swapMonthsAndSlide:direction keepOneRow:keepOneRow];
    ...
    // Remove Default Selection for First Date
